...
let karrierKrapp= async () => {
  try {
    ...
    await page.goto("https://karriere.krupp-krankenhaus.de/stellenangebote.html", {
      waitUntil: "load",
      timeout: 0,
    });
    ...
    const jobLinks = await page.evaluate(() => {
      return Array.from(
        document.querySelectorAll("td.real_table_col1 a")
      ).map((el) => el.href);
    });

    let titles = await page.evaluate(() => {
        return Array.from(document.querySelectorAll(".real_table_col1 a")
        ).map(el => el.innerText);
      });
      console.log(titles)
