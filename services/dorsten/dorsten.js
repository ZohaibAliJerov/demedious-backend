...
let dorsten = async () => {
  try {
    ...
    const jobLinks = await page.evaluate(() => {
      return Array.from(document.querySelectorAll("div.title > h2 > a")).map(
        (el) => el.href
      );
    });
    ...
    for (let jobLink of jobLinks) {
      ...
      let title = await page.evaluate(() => {
        let ttitle = document.querySelector("div.stelle.clear > h1");
      ...
      let link = await page.evaluate(() => {
          let apply = document.querySelector("div.text > a")
