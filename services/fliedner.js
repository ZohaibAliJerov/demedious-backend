...
let good = async () => {
  try {
    ...
    const jobLinks = await page.evaluate(() => {
      return Array.from(
        document.querySelectorAll(".hl3 a")
      ).map((el) => el.href);
    });
    ...
    for (let jobLink of jobLinks) {
      ...
      let title = await page.evaluate(() => {
        let ttitle = document.querySelector(".hl2");
      ...
      // let loc = document.querySelector(".con-content").innerText;
      // loc = loc.replace("\n", " ");
      ...
      let link = await page.evaluate(() => {
       let app = document.querySelector(".send-txt a");
